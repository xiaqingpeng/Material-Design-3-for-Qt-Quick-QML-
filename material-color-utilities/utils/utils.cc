...
std::string HexFromArgb(Argb argb) {
  ...
  ss << "#";
  char buffer[10];
  if (AlphaFromInt(argb) != 255) {
      sprintf(buffer, "%02X", AlphaFromInt(argb));
      ss << buffer;
  }
  sprintf(buffer, "%02X", RedFromInt(argb));
  ss << buffer;
  sprintf(buffer, "%02X", GreenFromInt(argb));
  ss << buffer;
  sprintf(buffer, "%02X", BlueFromInt(argb));
  ss << buffer;
  return ss.str();
}
